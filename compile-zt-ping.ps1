# Compile zt-ping.exe with MinGW g++
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Compiling zt-ping.exe" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Link zt-ping.exe with g++
Write-Host "Linking zt-ping.exe..." -ForegroundColor Yellow
...
& $MINGW_GPP @CFLAGS @INCLUDES -o "build\zt-ping.exe" "zt-ping.c" @LDFLAGS

if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "✓✓✓ Compilation SUCCESS!" -ForegroundColor Green
    Write-Host ""
    Get-Item "build\zt-ping.exe" | Select-Object Name, @{N='Size(KB)';E={[math]::Round($_.Length/1KB,2)}}, LastWriteTime
} else {
    Write-Host ""
