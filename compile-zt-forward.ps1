# Compile zt-forward.exe with MinGW g++
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "Compiling zt-forward.exe" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# Link zt-forward.exe with g++
Write-Host "Linking zt-forward.exe..." -ForegroundColor Yellow
...
& $MINGW_GPP @CFLAGS @INCLUDES -o "build\zt-forward.exe" "zt-forward.c" @LDFLAGS

if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "✓✓✓ Compilation SUCCESS!" -ForegroundColor Green
    Write-Host ""
    Get-Item "build\zt-forward.exe" | Select-Object Name, @{N='Size(KB)';E={[math]::Round($_.Length/1KB,2)}}, LastWriteTime
} else {
    Write-Host ""
